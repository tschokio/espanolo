...
const GROUP_TOPICS = new Map([
  ["b2-discourse-context-vocabulary", "b2-discourse-connectors"],
  ["b2-reported-context-vocabulary", "b2-reported-speech"],
  ["b2-relative-context-vocabulary", "b2-relative-clauses"],
  ["b2-se-context-vocabulary", "b2-se-constructions"],
  ["b2-past-subjunctive-context-vocabulary", "b2-past-subjunctive"],
  ["b2-periphrasis-context-vocabulary", "b2-verbal-periphrases"],
  ["b2-reading-context-vocabulary", "b2-reading-inference"],
  ["b2-listening-context-vocabulary", "b2-listening-comprehension"]
]);

test("B2 vocabulary expands through eight topic-bound contextual domains", async () => {
  ...
  try {
    ...
    for (const group of groups) {
      ...
      assert.equal(group.lessons.length, 6, `${group.slug} should attach to exactly one six-lesson B2 unit`);
      assert.equal(group.lessons.every((lesson) => lesson.cefrLevel === "B2"), true);
...
test("the full seed restores contextual B2 vocabulary after every B2 topic seed", () => {
  ...
  const expansionPosition = command.indexOf("seed-b2-context-vocabulary.js");
  const lastB2TopicPosition = Math.max(
    command.indexOf("seed-b2-listening-comprehension.js"),
    command.indexOf("seed-b2-connected-input-past-periphrases.js")
  );
  const practicePosition = command.indexOf("seed-lesson-practice.js");
  assert.ok(expansionPosition > lastB2TopicPosition);
  assert.ok(expansionPosition < practicePosition);
});

test("bounded B2 lesson batches introduce every topic word before the checkpoint", async () => {
  const prisma = new PrismaClient();
  try {
...
test("every contextual B2 meaning has an authored German bridge", async () => {
